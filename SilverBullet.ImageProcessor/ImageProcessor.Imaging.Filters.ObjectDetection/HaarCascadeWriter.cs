...
public class HaarCascadeWriter
{
	...
	public void Write(HaarCascade cascade, string className)
	{
		...
		for (int k = 0; k < cascade.Stages.Length; k++)
		{
			writeStage(k, cascade.Stages[k]);
		}
		writer.WriteLine();
	...
	private void writeStage(int i, HaarCascadeStage stage)
	{
		...
		writer.WriteLine("            stage = new HaarCascadeStage({0}, {1}, {2}); nodes = new List<HaarFeatureNode[]>();", stage.Threshold.ToString("R", NumberFormatInfo.InvariantInfo), stage.ParentIndex, stage.NextIndex);
	...
	{
		writer.Write("new HaarFeatureNode({0}, {1}, {2}, ", node.Threshold.ToString("R", NumberFormatInfo.InvariantInfo), node.LeftValue.ToString("R", NumberFormatInfo.InvariantInfo), node.RightValue.ToString("R", NumberFormatInfo.InvariantInfo));
		if (node.Feature.Tilted)
		{
			writer.Write("true, ");
		}
		for (int i = 0; i < node.Feature.Rectangles.Length; i++)
		{
			writeRectangle(node.Feature.Rectangles[i]);
			if (i < node.Feature.Rectangles.Length - 1)
			{
				writer.Write(", ");
			}
		}
