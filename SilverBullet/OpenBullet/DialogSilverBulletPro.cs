...
public class DialogSilverBulletPro : Page, IComponentConnector
{
	...
	public void InitializeComponent()
	{
		...
		{
			_contentLoaded = true;
			Uri resourceLocator = new Uri("/SilverBullet;component/views/dialogs/dialogsilverbulletpro.xaml", UriKind.Relative);
	...
	void IComponentConnector.Connect(int connectionId, object target)
	{
		...
		{
		case 1:
			((Button)target).Click += Buy_Click;
			break;
		case 2:
			((Button)target).Click += Channel_Click;
			break;
		case 3:
			((Image)target).MouseDown += Image_MouseDown;
			break;
		case 4:
			((Button)target).Click += Buy1_Click;
			break;
		case 5:
			((TextBox)target).Loaded += TextBox_Loaded;
			break;
		default:
			_contentLoaded = true;
			break;
