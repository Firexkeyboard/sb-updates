...
public class ScriptCompiler : IDisposable
{
	...
	public static string AssemblyVersion = "0.3.0.0";

	public static string AssemblyFileVersion = "0.3.0.0";
	...
	private string beginMainMethod = "static void Main(string[] args)\r\n        {\r\n\r\n            Console.Clear();\r\n\r\n            WriteName();\r\n            Console.WriteLine();\r\n            Console.WriteLine();\r\n            Console.WriteLine(\" [+] SB CLI by MOHAMM4DX\", Color.White);\r\n\r\n   ";
	...
	{
		AddOptions(new string[1] { option });
	}
	...
	public void AddOptions(string[] options)
	{
		...
		{
			CompilerParameters options2 = Options;
			options2.CompilerOptions = options2.CompilerOptions + text + " ";
		}
	}
	...
	public CompilerResults Execute()
	{
		...
		AddMethods(new string[1] { "\r\nprivate static void FoundHit(IRunnerMessaging obj, Hit hit)\r\n        {\r\n\r\n            // If an output file was specified, print them to the output file as well\r\n            if (outFile != string.Empty)\r\n            {\r\n                lock (FileLocker.GetLock(outFile))\r\n                {\r\n                    File.AppendAllText(outFile, $\"[{ DateTime.Now}]" + HitInformationFormat + "{Environment.NewLine}\");\r\n                }\r\n            }\r\n        }" });
		string invokeLic = string.Empty;
		if (!string.IsNullOrWhiteSpace(LicenseSource))
		{
			MatchCollection matchCollection = Regex.Matches(LicenseSource, "using(.*?);");
			if (matchCollection.Count > 0)
			{
				IEnumerable<string> source = from Match m in matchCollection
					select m.Groups[1].Value;
				source.ToList().ForEach(delegate(string u)
				{
					LicenseSource = LicenseSource.ReplaceFirst("using" + u + ";", string.Empty);
				});
				AddUsings(source.ToArray());
			}
			invokeLic = LicenseSource.Split(new string[1] { "Invoke]" }, StringSplitOptions.RemoveEmptyEntries)[1].Split('(')[0].Replace("\r\n", string.Empty);
			if (!string.IsNullOrWhiteSpace(invokeLic))
			{
				List<string> list = LicenseSource.Split(new string[1] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries).ToList();
				int num = list.IndexOf(list.First((string l) => l.StartsWith(invokeLic)));
				list.RemoveAt(num - 1);
				LicenseSource = string.Join("\r\n", list.ToArray());
			}
			invokeLic = invokeLic.Split(' ').Last();
			AddMethods(new string[1] { LicenseSource }, newLine: true);
		}
		return Compiler.CompileAssemblyFromSource(Options, usings + assemblies + @namespace + programClass + fields + properties + optionsClass + beginMainMethod + ((!string.IsNullOrWhiteSpace(invokeLic)) ? (invokeLic + "();") : string.Empty) + endMainMethod + methods + beginRunMethod + endRunMethod + endSource);
	}

	public (string, bool) GetResult(CompilerResults compilerResults)
	{
		string pathToAssembly = compilerResults.PathToAssembly;
		if (!compilerResults.Errors.HasErrors)
		{
			return ("Compiled Successfully On Path: " + pathToAssembly, false);
		}
	...
	{
		string[] array = new string[Options.ReferencedAssemblies.Count];
		Options.ReferencedAssemblies.CopyTo(array, 0);
		List<string> list = array.Where((string r) => File.Exists(r)).ToList();
		if (Config.SeleniumPresent)
		{
			list.AddRange(from f in DirExtensions.GetFiles("bin", "*.exe", SearchOption.TopDirectoryOnly)
				where f.ToLower().EndsWith("driver")
				select f);
		}
		if (ScriptExtension.HasBlock(Config.Script, "CloudScraper"))
		{
			list.Add("bin\\CloudScraper.exe");
		}
		for (int i = 0; i < list.Count; i++)
		{
			try
			{
				string text = "Compiled\\bin\\" + Path.GetFileName(list[i]);
				if (!File.Exists(text))
				{
					File.Copy(list[i], text);
				}
			}
			catch
			{
		...
		try
		{
			...
			{
				if (!Directory.Exists("Compiled\\bin\\tessdata"))
				{
					Directory.CreateDirectory("Compiled\\bin\\tessdata");
				}
				File.Copy(sourceFileName, "Compiled\\bin\\tessdata");
			}
		}
		catch
		{
	...
	public void CopySettings()
	{
		...
		if (!Directory.Exists("Compiled\\Settings"))
		{
			Directory.CreateDirectory("Compiled\\Settings");
		}
		string[] files = Directory.GetFiles("Settings");
		foreach (string text in files)
		{
			string fileName = Path.GetFileName(text);
			if (!File.Exists("Compiled\\Settings\\" + fileName))
			{
				File.Copy(text, "Compiled\\Settings\\" + fileName);
			}
		}
	}
	...
	public void CreateRunner(string name, Config config)
	{
		...
		{
			string text = "\"bin\\" + name + ".exe\"";
			File.WriteAllText("Compiled\\" + name + " Runner.bat", "@echo off\r\n" + text + " --bots 1 --output \"hits.txt\" --useproxies False --wordlist \"<WORDLIST PATH>\" --wltype Default\r\npause");
		}
		else
		{
			string text2 = "\"bin\\" + name + ".exe\"";
			File.WriteAllText("Compiled\\" + name + " Runner.bat", "@echo off\r\n" + text2 + " --bots 1 --output \"hits.txt\" --proxies \"<PROXY PATH>\" --ptype Http --useproxies True --wordlist \"<WORDLIST PATH>\" --wltype Default\r\npause");
		}
	}
	...
	private string ToLiteral(string input)
	{
		...
		Compiler.GenerateCodeFromExpression(new CodePrimitiveExpression(input), stringWriter, null);
	...
	public void AddUsings(string[] usings)
	{
		...
		{
			string usi = "using " + text.Trim() + ";";
			if (!this.usings.Split(new string[1] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries).Any((string u) => u.Trim() == usi))
			{
				this.usings = this.usings + usi + "\r\n";
			}
		}
	}
	...
	public void AddFields(string[] fields, bool newLine = false)
	{
		...
		{
			this.fields = this.fields + text + ((!newLine) ? string.Empty : "\r\n");
		}
	}
	...
	public void AddProperties(string[] properties, bool newLine = false)
	{
		...
		{
			this.properties = this.properties + text + ((!newLine) ? string.Empty : "\r\n");
		}
	}
	...
	public void AddMethods(string[] methods, bool newLine = false)
	{
		...
		{
			this.methods = this.methods + text + ((!newLine) ? string.Empty : "\r\n");
		}
	}
}
